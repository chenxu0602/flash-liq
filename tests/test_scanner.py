...
def test_normalize_flags_complex_erc4626_collateral() -> None:
    ...
    assert position["complex_collateral"]
    assert position["complex_reasons"] == ["tags:erc4626,vault-v1", "warnings:not_whitelisted"]
    assert position["pre_liquidations"][0]["address"] == "0xpreliq"


def test_filter_matches_symbol_and_complex_only() -> None:
    position = {
    ...
    assert matches_position_filters(
        position,
        ScanFilters(
            chain_id=1,
...
def test_filter_rejects_simple_collateral_when_complex_only() -> None:
    position = {
    ...
    assert not matches_position_filters(
        position,
